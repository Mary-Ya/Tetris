...
Tetromino.prototype.move = function(ofsX, ofsY, scene) {
    if (round.over !== true && round.pause !== true) {
        ...
        scene.mergeWith(curTetro);
        if (cantMove == false) {
            clean();
            curTetro.X = nextX; // просто меняем координаты
            curTetro.Y = nextY;
            scene.mergeWith(this);
        } else { // 
    ...
    for (var i = 0; i < tetromino.fig.length; i++)
        for (var j = 0; j < tetromino.fig[i].length; j++) {
            if (tetromino.fig[i][j] !== 0)
				var aim = this.get();
				aim[tetromino.Y + i][tetromino.X + j] += tetromino.fig[i][j];
				this.set(aim);
        };
